...
from visualnav_transformer.deployment.src.topic_names import IMAGE_TOPIC, WAYPOINT_TOPIC
        ...
        if len(msg.data) >= 2:
            ...
            if self.latest_image is not None:
                self.latest_waypoint = (
                    int(msg.data[0] * self.latest_image.shape[0]),
                    int(msg.data[1] * self.latest_image.shape[1]),
                )
    ...
    def process_and_publish(self):
        if self.latest_image is not None and self.latest_waypoint is not None:
            ...
            self.get_logger().info(
                f"Published image with waypoint at {self.latest_waypoint}"
            )
